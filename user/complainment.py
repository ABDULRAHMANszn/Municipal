...
from db_manager import insert_complaint, get_user_id


class Complainment(QMainWindow):
    def __init__(self, username):
        ...
        self.resize(910, 840)
        self.setWindowTitle("Complainment")
        self.setWindowIcon(QIcon("../images/icon.png"))

        self.welcomL = QLabel("Make a Complainment", self)
        self.label1 = QLabel("Title :", self)
        self.label2 = QLabel("Contact Information :", self)
        self.label3 = QLabel("Descripition :", self)
        self.label4 = QLabel("Complaint Type :", self)
    ...
    def GU_Utin(self):
        ...
        self.field2.setGeometry(450, 300, field_width, 35)
        self.field3.setGeometry(450, 360, field_width, 35)
        self.combo_box.setGeometry(450, 420, field_width, 35)
        ...
        self.combo_box.setStyleSheet("""
            ...
            color: white;
        """)
        self.combo_box.addItems(["Water", "Electricity", "Sanitation", "Street Lighting"])
...
font-size: 20px;
    ...
    def Sendbutton(self):
        title = self.field1.text().strip()
        contact = self.field2.text().strip()
        description = self.field3.text().strip()
        complaint_type = self.combo_box.currentText()

        if not title or not contact or not description:
            QMessageBox.warning(self, "Input Error", "Please fill in all the fields.")
            return

        insert_complaint(self.user_id, title, contact, description, complaint_type)
